#!/usr/bin/env python3
"""資料源連線診斷 — 檢查找買點管線的所有外部依賴

用法：
    python3 scripts/check_data_sources.py

每個來源顯示 ✅（可用）/ ⚠️（有回應但無資料）/ ❌（不可用），
並附管線用途說明。退出碼：全部核心來源可用=0，任一核心失敗=1。
"""
from __future__ import annotations

import datetime as dt
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

import requests

HEADERS = {"User-Agent": "Mozilla/5.0"}
# 核心來源失敗會影響管線主要功能
CORE = {"twse_openapi", "twse_t86", "yfinance", "finmind"}


def check_twse_openapi() -> str:
    """月營收 YoY／季 EPS 主路徑"""
    r = requests.get("https://openapi.twse.com.tw/v1/opendata/t187ap05_L",
                     headers=HEADERS, timeout=15)
    if r.status_code == 200 and isinstance(r.json(), list):
        return f"✅ OK（{len(r.json())} 筆月營收）"
    return f"⚠️ HTTP {r.status_code}"


def check_twse_t86() -> str:
    """三大法人籌碼主路徑。今日盤後 16:00 才發布，自動往回找最近交易日"""
    d = dt.date.today()
    tried = []
    for _ in range(7):
        if d.weekday() < 5:
            ds = d.strftime("%Y%m%d")
            j = requests.get(
                f"https://www.twse.com.tw/rwd/zh/fund/T86?date={ds}"
                "&selectType=ALL&response=json",
                headers=HEADERS, timeout=15).json()
            stat = j.get("stat", "")
            if stat == "OK":
                return f"✅ OK（{d}，{len(j.get('data', []))} 檔）"
            tried.append(f"{ds} 無資料")
        d -= dt.timedelta(days=1)
    return f"⚠️ 七日內無資料（{'；'.join(tried[:3])}…）；16:00 前跑當日屬正常"


def check_yfinance() -> str:
    """預估 EPS／日線主路徑"""
    import warnings
    warnings.filterwarnings("ignore")
    import yfinance as yf
    info = yf.Ticker("2330.TW").get_info()
    if info.get("symbol") or info.get("shortName"):
        eps = info.get("forwardEps")
        return f"✅ OK（2330 forwardEps={eps}）"
    return "⚠️ 有回應但無 symbol 資料"


def check_finmind() -> str:
    """備援：法人／營收／財報／日線。token 未填時為 guest 額度"""
    from common.cache import DiskCache
    from common.rate_limit import RateLimiter
    from common.finmind import FinMindClient
    import json as _json
    from pathlib import Path

    token = ""
    cfg = Path(__file__).resolve().parent.parent / "config_pipeline.json"
    if cfg.exists():
        token = _json.loads(cfg.read_text(encoding="utf-8")).get(
            "finmind_token", "") or ""

    rl = RateLimiter({"finmind": {"delay": 0.7, "jitter": 0.2}})
    fm = FinMindClient(token=token or None, rate_limiter=rl)
    rows = fm.fetch_dataset("TaiwanStockPrice", "2330",
                            start_date="2026-08-20", end_date="2026-08-25")
    fm.close()
    mode = "token" if token else "guest"
    if rows:
        return f"✅ OK（{mode}，{len(rows)} 筆日線）"
    return f"❌ 無資料（{mode}；402=額度用盡、401=token 無效）"


def check_moneydj() -> str:
    """族群分類（走 TWCA TLS 備援）"""
    from common.moneydj import fetch_industry_index
    from common.tls_fallback import make_twca_session
    index = fetch_industry_index(make_twca_session())
    if len(index) >= 1000:
        return f"✅ OK（{len(index)} 個產業分類）"
    if index:
        return f"⚠️ 僅 {len(index)} 分類（預期 ≥1000）"
    return "❌ 索引抓取失敗"


def check_tdcc() -> str:
    """大戶持股比率（集保；同樣踩 TWCA 缺 SKI，已內建備援）"""
    from common.tdcc import TDCCQuery
    from common.rate_limit import RateLimiter
    tdcc = TDCCQuery(RateLimiter({"tdcc": {"delay": 3.0, "jitter": 0.5}}),
                     retries=1)
    dates = tdcc.available_dates()
    if dates:
        return f"✅ OK（最新集保日 {dates[0]}）"
    return "❌ available_dates 失敗（SSL 備援亦未通）"


CHECKS = [
    ("twse_openapi", "TWSE OpenAPI（營收/EPS 主路徑）", check_twse_openapi),
    ("twse_t86", "TWSE fund/T86（籌碼主路徑）", check_twse_t86),
    ("yfinance", "yfinance（EPS 預估——唯一免費源）", check_yfinance),
    ("finmind", "FinMind（備援通道）", check_finmind),
    ("moneydj", "MoneyDJ（族群分類）", check_moneydj),
    ("tdcc", "TDCC 集保（大戶持股）", check_tdcc),
]


def main() -> int:
    print("找買點管線 — 資料源診斷\n" + "=" * 50)
    failed_core = []
    for key, desc, fn in CHECKS:
        try:
            result = fn()
        except Exception as e:  # noqa: BLE001
            result = f"❌ {type(e).__name__}: {str(e)[:60]}"
        print(f"\n[{key}] {desc}\n  → {result}")
        if result.startswith(("❌",)) and key in CORE:
            failed_core.append(key)

    print("\n" + "=" * 50)
    if failed_core:
        print(f"❌ 核心來源失敗：{', '.join(failed_core)}")
        print("   提示：T86『無資料』多為盤後 16:00 前前執行，屬正常；")
        print("   FinMind 402=額度用盡（免費 600/hr）、401=token 無效。")
        return 1
    print("✅ 全部核心來源可用")
    return 0


if __name__ == "__main__":
    sys.exit(main())
