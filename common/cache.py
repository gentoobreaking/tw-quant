...
PostgreSQL 支援 (T018)：當 DATABASE_URL 環境變數設定時，
快取自動切換到共享 PostgreSQL (pickup.cache 表格)，
使 tw-quant-pickup 成為 core schema 的唯一寫入者。
"""
...
class DiskCache:
    """SQLite/PostgreSQL 雙模式磁碟快取，具 TTL 過期機制

    當 DATABASE_URL 環境變數或 database_url 參數設定時，
    自動切換到 PostgreSQL (pickup.cache 表格) 作為快取後端。
    否則使用 SQLite (db_path)。
    ...
    def __init__(
        self, db_path: str, ttl: int = 7200, database_url: Optional[str] = None
    ):
        ...
        self._use_pg = False
        # 若 database_url 未傳入，檢查 DATABASE_URL 環境變數
    ...
    def _get_pg_conn(self):
        """取得 PostgreSQL 連線（惰性初始化 + 斷線重連）"""
        import psycopg

        if self._pg_conn is None:
            self._pg_conn = psycopg.connect(self._pg_url, autocommit=True)
        try:
            self._pg_conn.execute("SELECT 1")
        except Exception:
            self._pg_conn = psycopg.connect(self._pg_url, autocommit=True)
        return self._pg_conn

    def _ensure_pg_table(self):
        """建立 PostgreSQL cache 表格 (pickup.cache)"""
    ...
    def get(
        ...
        if self._use_pg:
            row = conn.execute(
                "SELECT data, ts FROM pickup.cache WHERE key = %s", (key,)
            ).fetchone()
        else:
            row = conn.execute(
        ...
        try:
            ...
            with self._write_lock:
                if self._use_pg:
                    conn.execute(
                        "INSERT INTO pickup.cache (key, data, ts) VALUES (%s, %s, %s) "
    ...
    def _cleanup_expired(self):
        ...
        with self._write_lock:
            conn = self._get_cache_conn()
            if self._use_pg:
                conn.execute("DELETE FROM pickup.cache WHERE ts < %s", (cutoff,))
            else:
    ...
    def load_disk_cache(self) -> dict:
        ...
        result = {}
        if self._use_pg:
            cur = conn.execute("SELECT key, data, ts FROM pickup.cache")
        else:
            cur = conn.execute("SELECT key, data, ts FROM cache")
        for key, data, ts in cur:
            try:
